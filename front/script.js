...
const getList = async () => {
    ...
    .then( (jsonData) => {
        jsonData.forEach( item => insertList(item.id, item.titulo, item.autor, item.editora));
    })
    .catch( (error) => {
...
const novoItem = async () => {
    ...
    let requestBody = {
        titulo: document.getElementById('newProduct').value,
        autor: document.getElementById('newQuantity').value,
        editora: document.getElementById('newPrice').value
    }
...
*/
const insertList = (id, titulo, autor, editora) => {
    var item = [id, titulo, autor, editora];
